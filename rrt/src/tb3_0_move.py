...
if __name__ == '__main__':
    try:
        ...
        rospy.init_node('tb3_0_move', anonymous=True)

        rate = rospy.Rate(10)
        vel_command = Twist()
        fname = '/home/sahruday/Documents/RRT/final_path1.txt'
        ...
        for p in femp:
            ...
            while(True):
                ...
                if abs(angle - yaw)> 0.05:
                    ...
                    print(abs(math.sqrt(((goal_x-(x)) ** 2) + ((goal_y-(y)) ** 2))),'distance','agent1')
                    vel_command.linear.x = 0.1
                    vel_command.angular.z = 0.0
                    pub.publish(vel_command)
                    print ('x=', (x), 'y=',(y), 'moving')
                    rate.sleep()
                elif 6.9 <= x <= 7.1 and 5.9 <= y <= 6.1:
