...
if __name__ == '__main__':
    try:
        sub = rospy.Subscriber("tb3_2/odom", Odometry, newOdom)
        pub = rospy.Publisher('tb3_2/cmd_vel', Twist, queue_size=10)
        rospy.init_node('tb3_2_move', anonymous=True)

        rate = rospy.Rate(10)
        vel_command = Twist()
        gname = '/home/sahruday/Documents/RRT/final_path3.txt'
            ...
            while(True):
                ...
                if abs(angle - yaw)> 0.05:
                    ...
                    if angle<yaw:
                        ...
                        pub.publish(vel_command)
                    rate.sleep()
                elif abs(math.sqrt(((goal_x-(x)) ** 2) + ((goal_y-(y)) ** 2)))>0.105:
                    print(abs(math.sqrt(((goal_x-(x)) ** 2) + ((goal_y-(y)) ** 2))),'distance','agent3')
                    vel_command.linear.x = 0.1
                    vel_command.angular.z = 0.0
                    pub.publish(vel_command)
                    print ('x=', (x), 'y=',(y), 'moving')
                    rate.sleep()
                elif 6.9 <= x <= 7.1 and 6.9 <= y <= 7.1:
